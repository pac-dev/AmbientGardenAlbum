/**
 * String-like polyphonic patch playing a slow and simple progression.
 * 
...
const bass = new FaustNode('faust/bass.dsp', { preamp: 1, freq: 0, noise1: 0, lp1: 700, lp2: 3500 });
...
const lp1Param = graph.addParam('lp1', { def: 700, min: 100, max: 15000 });
const lp2Param = graph.addParam('lp2', { def: 1200, min: 100, max: 15000 });
...
const mkVoice = i => {
	const ret = new FaustNode('faust/string.dsp', { freq: 500, noise: 0, lp1: 3300, lp2: 580 });
	lp1Param.connect(ret.lp1);
	lp2Param.connect(ret.lp2);
	ret.notePos = 100;
	ret.amp = 1;
	ret.note = (freq, amp) => {
...
// Use the Poly class to make an instrument with 10 managed voices
const poly = new Poly(10, mkVoice, post);
